...
int main() {
    ...
    while (cin >> n) {
        ...
        for (int i = 0; i < siz(available); i++) {
            if (available[i] == 0)
                continue;
            char c = (char)(available[i] + '0');
            int mod = available[i] % n;
            if (mod == 0) {
                ans = c;
                goto loop;
            }
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < siz(available); i++) {
                string now = x.ff;
                now += (char)(available[i] + '0');
                int mod = (x.ss * 10 + available[i]) % n;
                if (mod == 0) {
                    //bug(mod, x.ss, available[i]);
                    ans = now;
                    goto loop;
                }
                if (!vis[mod]) {
