...
pii ret(int l,int r){
    ...
    for(int mid=l;mid<r;mid++){
        ...
        pll y=ret(mid+1,r);
        ll temp=firr;
        firr=min(firr,x.ff+y.ff+x.ss*y.ss);

        if(temp!=firr){
            sec=(x.ss+y.ss)%100;
           // fir=x.ff+y.ff+firr;
        }
        else if(x.ff+y.ff+x.ss*y.ss==firr and sec>(x.ss+y.ss)%100){
            sec=(x.ss+y.ss)%100;
        }

    }
    return dp[l][r]=mp(firr,sec);
}
