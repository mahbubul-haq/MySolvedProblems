...
int main()
{
    ...
    for(int i=0;i<3;i++){
        double x,y,z;
        cin>>x>>y>>z;
        points.pb(Point3D(x,y,z));
    }

    double r; cin>>r;

    double ab=dist(points[0],points[1]);
    double ac=dist(points[0],points[2]);
    double bc=dist(points[1],points[2]);
    ...
    double AB=ac*cosA;
    double BA=bc*cosB;
    double dis=ac*((1-cosA*cosA)<=0? 0:sqrt(1-cosA*cosA));
    ...
    double chord=sqrt(r*r-dis*dis);
    double Bch=BA-chord;
    double Ach=AB-chord;
    double tangA=sqrt(Ach*(Ach+2*chord));
    double tangB=sqrt(Bch*(Bch+2*chord));

    double ang=acos(dis/ac)+acos(dis/bc);
    double angA=atan(tangA/r);
    double angB=atan(tangB/r);

    cout<<tangA+tangB+r*(ang-angA-angB)<<endl;
