...
void add(int node ,int l, int r,int i,int val) {
    ...
    int m= mid(l, r);
    add(left(node),l, m,i, val);
    add(right(node), m + 1, r,i, val);
...
ll query(int node, int l, int r,int x ,int y) {
    ...
    int m = mid(l, r);
    ll v = query(left(node), l, m, x , y);
    ll u = query(right(node), m + 1, r, x, y);
    return (u ^ v);
}
...
int main()
{
    //FI;//FO;
    //FastIO;//timeInit;
    const  int val =6 * 100005;

    st.assign( 4*val + 10, 0);

    int q; cin >> q;
    int le = 300001, ri = 300002;
    ...
    while (q-- > 0) {
        ...
        if(type== 1) {
            ...
            int x, y;
            cin >> x>>y;
            cout << query(1, 1, val, le + x, le + y) << endl;
        }
    }
