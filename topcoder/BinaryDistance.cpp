...
public:
    ...
    int maxDist (int n, int v) {
        ...
        ll v1 = power(2, llev), v2 = power(2, llev + 1) - 1;
        ...
        level--;
        ll val1 = power(2, level), val2 = power(2, level + 1) - 1;

        if (abs(v - val1) > abs(v - val2)) {
            return (int)(level + llev);
        }
        else if (abs(v - val1) < abs(v - val2)) {
            return (int) (level + rlev);
        }
        else return (int)llev;
    }
};
