...
int main() {
    ...
    dp[1][0] = (bigmod(k , n) - bigmod(k - 1, n) + MOD) % MOD;

    for (int r = 2; r <= n; r++) {
        dp[r][0] = dp[1][0] * dp[r - 1][0] % MOD;
    }
    ...
    for (int c = n - 2; c > 0; c--) {
        dp[1][c] = dp[1][c + 1] * dp[1][n - 1] % MOD;
    }

    for (int r = 2; r <= n; r++) {
        for (int c = 1; c <= n; c++) {
            dp[r][c] = (bigmod(k, n - c) - bigmod(k - 1, n - c) + MOD) % MOD * dp[r - 1][c] % MOD * bigmod(k - 1, c) % MOD;
            ll ans = 0, mul = bigmod(k, n - c);
            for (int cc = c; cc >= 1; cc--) {
                ans = (ans + mul * fact[c] % MOD * factinv[c - cc] % MOD * factinv[cc] % MOD * dp[r - 1][c - cc] % MOD) % MOD;
                mul = mul * (k - 1) % MOD;
            }
            dp[r][c] = (dp[r][c] + ans) % MOD;
        }
    }
