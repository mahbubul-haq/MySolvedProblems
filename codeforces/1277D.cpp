...
int main() {
    ...
    while (t--) {
        ...
        vector<pair<string, int> > oz, zo;
        unordered_map<string, bool> tak;
        int nn = n;
        int i = 1;
        while (i <= n) {
            string s;
            ...
            else if(s[0] == '1' and s[l] == '1') {
                oo = true;
            }
            else if (s[0] == '1' and s[l] == '0') {
                oz.pb({s, i});
            }
            else zo.pb({s, i});
            i++;
        ...
        if ((zz or oo) and siz(oz) == 0 and siz(zo) == 0) {
            if (zz and oo)
                ok = false;
        }
        else {
 
            int cnz = siz(oz), cnto = siz(zo);
 
            if (cnz - cnto > 1) {
 
                int j = 0;
                while (cnz - cnto > 1 and j < siz(oz)) {
                    string tem = oz[j].ff;
                    reverse(all(tem));
                    if (!tak[tem]) {
                        tak[tem] = true;
