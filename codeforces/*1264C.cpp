...
ll prob[maxn + 1];
int p[maxn + 1];
ll pre[maxn + 1], mul[maxn+ 1], ans;

ll getmul(int l, int r) {
    return (mul[r] * bigmod(mul[l - 1], MOD - 2) % MOD);
}

void add(int l, int r, int sign) {
    ans = (ans + sign * (pre[r - 1] - getmul(l, r - 1) * pre[l - 1] % MOD) + MOD) % MOD;
}
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        cin >> p[i];
        prob[i] = bigmod(p[i], MOD - 2) * 100 % MOD;
    }
    ...
    mul[0] = 1;
    for (int i = 1; i <= n; i++) {
        pre[i] = (pre[i - 1] * prob[i] + prob[i]) % MOD;
        mul[i] = mul[i - 1] * prob[i] % MOD;
    }
