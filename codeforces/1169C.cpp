...
bool solve(int mid){
    if(ara[n]+mid>=m) temp[n]=m-1;
    else temp[n]=ara[n]+mid;

    for(int i=n-1;i>=1;i--){
        if(ara[i]+mid<m){
            if(ara[i]<=temp[i+1])temp[i]=min(ara[i]+mid,temp[i+1]);
            else return false;
        }
        else{
            if(ara[i]<=temp[i+1]) temp[i]=temp[i+1];
            else temp[i]=min(temp[i+1],(ara[i]+mid)%m);
        }
    }
