...
void Search(int node, int l, int r, int i, int j, bool add) {
    ...
    if (st[node] or add) {
        if (l == r) {
            ...
            int md = mid(l, r);
            Search(left(node), l, md, i, j, add);
            Search(right(node), md + 1, r, i, j, add);
...
int main() {
    ...
    while (m--) {
        ...
        l--, r--;
        now.clear();
        Search(1, 0, n - 1, l, r, false);
        for (auto &xx : now) {
            lostto[xx] = x;
        }
        Search(1, 0, n - 1, x - 1, x - 1, true);
    }

    for (int i = 1 ; i <= n; i++) {
        cout << (lostto[i] == i ? 0 : lostto[i]) << " ";
    }
