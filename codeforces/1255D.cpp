...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i <min(26, k); i++) {
            ch += (char)('a' + i);
        }
 
        for (int i = 26; i < min(52, k); i++) {
            ch += (char) ('A' + i - 26);
        }
 
        for (int i = 52; i < min(62, k); i++)
            ch += (char) ('0' + i - 52);
        ...
        vis.assign(r + 1, vi(c + 1, -1) );
 
        int avg = siz(rrr) / k;
        int excess = siz(rrr) - avg * k;
 
        int now = avg + (excess  > 0? 1 : 0), i = 1, j = 1, cc = 0;
        excess--;
        int cn = 0;
        while (true) {
            cn++;
