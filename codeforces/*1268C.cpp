...
void add(int id, int x, int val) {
    for (int i = x; i < siz(ft[id]); i += LSOne(i)) {
        ft[id][i] += val;
        //bug(i, x);
    }
}

ll pref(int id, int x) {
    ll sum = 0;
...
ll rangesum(int id, int a, int b) {
    return pref(id, b) - pref(id, a - 1);
}
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        int p = pos[i], right = i - 1 - pref(0, p);
        //bug(i, right);
        add(0, p, 1);
        os.insert(p);
        add(1, p, p);
        tot += p;
        ll po = *os.find_by_order(i / 2);
        ll leftsum = pref(1, po) - po, l = i / 2;
        ll rightsum = tot - leftsum - po, r = i - 1 - l;

        now = right;
        last += now;

        //bug(po, i, l, r, leftsum, rightsum);

        if (l > 0) {
            ll need = po * (po - 1) / 2 - (po - l - 1) * (po - l) / 2;
            now += need - leftsum;
        }
        if (r > 0) {
            ll need = (po + r) * (po + r + 1) / 2 - po * (po + 1) / 2;
            now += rightsum - need;
        }
        //bug(last, now);
        pos[i] = last - right + now;
