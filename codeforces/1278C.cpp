...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 1; i <= n + n; i++) {
            ...
            a[i] = x;
            if (x &1 ){
                sum[i].ff = sum[i - 1].ff + 1;
                sum[i].ss = sum[i - 1].ss;
            }
            else {
                sum[i].ss = sum[i - 1].ss + 1;
                sum[i].ff = sum[i - 1].ff;
            }
        }
 
        int onee = sum[2 * n].ff, twoo = sum[2 * n].ss;
 
        unordered_map<int, int> mpp, mppl;
        int one = 0, two = 0;
        for (int i = n; i >= 1; i--) {
            if (a[i] == 1) {
                one++;
        ...
        int val = onee - twoo;
            ...
            if (present(mpp, onee - twoo)) {
                mini = min(mini, n - mpp[val] + 1);
            }
        ...
        for (int l = 1; l <= n; l++) {
            ...
            if (present(mpp, val - dif)) {
                mini = min(mini, l + n - mpp[val - dif] + 1);
            }
        }
