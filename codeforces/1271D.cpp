...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        if (to[i] != 0) {
            from[to[i]].insert({castle[i - 1].ss.ss, i});
        }
    }
    ...
    for (int i = 1; i <= n; i++) {
        ...
        int a = castle[i - 1].ff, b = castle[i - 1].ss.ff;

        if (k < a) {
            while (k < a and !st.empty())
                st.erase(st.begin()), k++;
            if (k < a) {
                ok = false;
                break;
        ...
        if (to[i] == 0) {
            from[i].insert({castle[i - 1].ss.ss, i});
        }
        auto it = from[i].end();

        if (!from[i].empty() and k > 0) {
            do {
                it--;
                st.insert(*it);
                k--;
            } while (k > 0 and it != from[i].begin());
        }
    }
