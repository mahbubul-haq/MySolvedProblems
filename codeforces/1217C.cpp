...
int main()
{
    ...
    while(t--){
        string s; cin>>s;
        ll ans =0;
        multiset<int> mset,mset1;
        for(int i =0;i<siz(s);i++){
            if(s[i]=='1')
            {
                mset1.insert(i);
                ans++;
                if(i<siz(s)-1 and s[i+1] =='0')
                    ans++;
            }
            else mset.insert(i);
        }
        ...
        for(int i=1;i<siz(s);i++){
            if(s[i]=='1'){
                auto it=mset.upper_bound(i);
                auto it1=mset1.upper_bound(i-1);
                ...
                if(it==mset.begin()){
                    ...
                    if(one >= i-1)
                        ...
                        maxi++;
                        now+=(s[i]-'0');
                        for(int j = 1;j<25 and i+j<siz(s);j++){
                            now*=2;
                            now+=(s[j+i]-'0');
                            maxi++;
                            if(now> j+1 and now <= maxi)
                                ans++;
                            else if(now > maxi) {
                                break;
                            }
