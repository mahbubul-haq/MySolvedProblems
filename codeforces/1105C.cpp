...
ll ret(int id,int mod){
    ...
    ll val1=ff*ret(id+1,mod);
    ll val2=ss*(ret(id+1,(mod+1)%3));
    ll val3=tt*(ret(id+1,(mod+2)%3));

    return dp[id][mod]=(val1+val2+val3)%MOD;
}
...
int main()
{
    ...
    cout<<(ff*ret(2,0)+ss*ret(2,1)+tt*ret(2,2))%MOD<<endl;
