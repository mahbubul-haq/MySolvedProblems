...
//    fap(p);
    ...
    for(int i=1;i<n;i++){
        for(int j=0;j<=p;j++){
            for(int k=0;k<2;k++){
                dp[i+1][j][k]=dp[i+1][j][k]+((f(j,k)-i)*(dp[i][j][k])%MOD);
                dp[i+1][j][k]%=MOD;
                if(j) dp[i+1][j-1][k]=dp[i+1][j-1][k]+((f(j-1,k)-f(j,k))*dp[i][j][k]%MOD);
                dp[i+1][j-1][k]%=MOD;
                if(k) dp[i+1][j][k-1]=dp[i+1][j][k-1]+((f(j,k-1)-f(j,k))*dp[i][j][k]%MOD);
                dp[i+1][j][k-1]%=MOD;
            }
        }
    }
