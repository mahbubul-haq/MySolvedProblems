...
///idea: https://www.youtube.com/watch?v=IFNibRVgFBo&list=PLrmLmBdmIlpuE5GEMDXWf0PWbBD9Ga1lO&index=10
...
public:
	...
	int maxCoins(vi &nums) {
		...
		for (int i = 0; i < n; i++) {
			dp[i][1] = nums[i];
            dp[i][1] *= (i > 0 ? nums[i - 1] : 1);
            dp[i][1] *= (i < n - 1 ? nums[i + 1] : 1);
        }
		...
		for (int len = 2; len <= n; len++) {
			for (int i = 0; i <= n - len; i++) {
				...
				for (int j = i; j < i + len; j++) {
					int val = nums[j];
					val *= (i + len >= n ? 1 : nums[i + len]);
					val *= (i > 0 ? nums[i - 1] : 1);
					val += (j != i ? dp[i][j - i] : 0);
					val += (j != i + len - 1 ? dp[j + 1][i + len - j - 1] : 0);
					maxi = max(maxi, val);
				}
				dp[i][len] = maxi;
			}
