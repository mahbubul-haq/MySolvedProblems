...
public:
    vector<string> fullJustify(vector<string>& words, int maxWidth) {
        ...
        while(true){
            ...
            else if(len+sz+siz(words[i])>maxWidth){
                string temp;
                if(sz==1){
                    temp=dq.front();
                    for(int l=1;l<=maxWidth-siz(dq.front());l++)
                        temp+=" ";
                }
                else{
                    sz--;
                    ...
                    blank=blank/sz;
                    int extra=maxWidth-len-sz*blank;
                    while(!dq.empty()){
                        string t=dq.front();
                        dq.pop_front();
                        temp+=t;
