...
int main() {
    ...
    factinv[100000] = bigmod(fact[100000], MOD - 2) % MOD;
    ...
    for (int i = k - 1; i < n; i++) {
        ans = (ans + nCr(i, k - 1) * ara[i] % MOD) % MOD; /// how many times ara[i] becames max in the chosen subarray * ara[i]
    }

    for (int i = 0; i <= n - k; i++) {
        ans = (ans - nCr(n - i - 1, k - 1) * ara[i] % MOD + MOD) % MOD; /// how many times ara[i] becomes minimum
    }
