...
int main()
{
    ...
    for(int i=n;i<2*n;i++){
        ans+=((i*nCr(i-1,n-1)%MOD)*((bigmod(a*bigmod(a+b,MOD-2,MOD)%MOD,n,MOD)*bigmod(b*bigmod(a+b,MOD-2,MOD)%MOD,i-n,MOD)+
                bigmod(b*bigmod(a+b,MOD-2,MOD)%MOD,n,MOD)*bigmod(a*bigmod(a+b,MOD-2,MOD)%MOD,i-n,MOD))%MOD))%MOD;
        ans%=MOD;
    }
