...
int main()
{
    ...
    for(int i=0;i<E;i++){
        int u,v,w;
        cin>>u>>v>>w;
        AdjLst[u].pb(pii(v,w-p));///directed graph....
    }

    vll dist(V+1,INT_MAX);
    dist[source]=0;
    int dir[V+1];
    dir[source]=-1;
    ...
    bool isNegativeCycle=false,isN=false;
    ...
    while(ii--) {
        for (int i = 1; i <= V; i++) {
            ...
            if (isNegativeCycle and isN) break;
        }
    }

    if((isNegativeCycle and isN) or dist[n]==INT_MAX)
    {
        cout<<-1<<endl;
        exit(0);
    }
