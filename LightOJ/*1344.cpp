...
int rec(int i, int l, int r) {
    ...
    int ret = 0;
    while(vis[ret]) ret++;
    return grundy[i][l][r] = ret;
}

void aladdin(int xorsum) {
    vii ans;
    for (int i = 0; i < n; i++) {
        int temp = xorsum;
        ...
        for (int j = 0; j < siz(vec[i]); j++) {
            temp = xorsum ^ rec(i, 0, siz(vec[i]) - 1);
            if (!taken[vec[i][j]]) {
                taken[vec[i][j]] = true;
                int pos = 0;
                for (int jj = 0; jj < siz(vec[i]); jj++) {
                    if (vec[i][jj] >= vec[i][j]) {
                        temp ^= rec(i, pos, jj - 1);
                        pos = jj + 1;
                    }
                }
                temp ^= rec(i, pos, siz(vec[i]) - 1);
                if (temp == 0)
                    ans.pb({i + 1, vec[i][j]});
            }
        }
