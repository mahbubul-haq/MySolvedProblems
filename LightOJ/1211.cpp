...
int main()
{
    ...
    for(int i=1;i<=t;i++){
        ...
        int n; cin>>n;
        pair<pii,int> lowl,upr;
        cin>>lowl.ff.ff>>lowl.ff.ss>>lowl.ss;
        cin>>upr.ff.ff>>upr.ff.ss>>upr.ss;

        for(int j=1;j<n;j++){
            int x1,y1,z1,x2,y2,z2;
            cin>>x1>>y1>>z1>>x2>>y2>>z2;
            lowl.ff.ff=max(lowl.ff.ff,x1);
            lowl.ff.ss=max(lowl.ff.ss,y1);
            lowl.ss=max(lowl.ss,z1);
            upr.ff.ff=min(upr.ff.ff,x2);
            upr.ff.ss=min(upr.ff.ss,y2);
            upr.ss=min(upr.ss,z2);
        }

        if(lowl.ff.ff>=upr.ff.ff or lowl.ff.ss>=upr.ff.ss or lowl.ss>=upr.ss){
            cout<<0<<endl;
        }
        else{
            ll ans=(ll)(upr.ff.ff-lowl.ff.ff)*(ll)(upr.ff.ss-lowl.ff.ss)*(upr.ss-lowl.ss);
            cout<<ans<<endl;
        }
