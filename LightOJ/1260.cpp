...
int main()
{
    ...
    while(t--){
        ...
        stadium.pb(stadium[0]);

        for(int i=0;i<siz(points);i++){
            Point prev=stadium[0];
            for(int j=1;j<siz(stadium);j++){
                mini=min(mini,distToLineSegment(points[i],prev,stadium[j],c));
                prev=stadium[j];
            }
        }
        points.pb(points[0]);

        for(int i=0;i<siz(stadium)-1;i++){
            Point prev=points[0];
            for(int j=1;j<siz(points);j++){
                mini=min(mini,distToLineSegment(stadium[i],prev,points[j],c));
                prev=points[j];
            }
        }
