...
int main()
{
    ...
    while(t--){
        ...
        for(int i=0;i<n;i++){
            ...
            vector<Point> points;
            Point c;
            for(int j=0;j<siz(lines);j++){
                if(areIntersect(lines[j],l1,c)){
                    if(l-c.x>EPS and c.x>EPS and w-c.y>EPS and c.y>EPS){
                        points.pb(c);
                    }
                }
            }
            ...
            for(int j=1;j<siz(points);j++){
                if(abs(c.x-points[j].x)<EPS and
                abs(c.y-points[j].y)<EPS){
                    //cntt++;
                }
                else c=points[j],cntt++;
            }
            cnt+=cntt;
            //cout<<cnt<<endl;
        }
