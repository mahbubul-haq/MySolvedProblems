...
void primedivs(ll a){
    ...
    if(a>1) factors.pb(mp(a,1));
}

int numOfPair(int idx,ll div,ll low,ll hi){
    if(idx==siz(factors)){
        if(div>=low and div<=hi){
            return 1;
        }
        return 0;
    }
    ll ans=0;
    for(int i=0;i<=factors[idx].ss;i++){
        ans+=numOfPair(idx+1,div,low,hi);
        div*=factors[idx].ff;
    }
    return ans;
}
...
int main()
{
    ...
    for(int i=1;i<=t;i++){
        ...
        cin>>a>>b;
        primedivs(a);
        cout<<numOfPair(0,1,b,a/b)/2<<endl;
