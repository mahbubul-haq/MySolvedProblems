...
void phi(ll n,ll m){
    ...
    while(pf*pf<=n){
        if(n%pf==0){
            ...
            sumofDivs=((sumofDivs*(bigmod(pf,cnt*m+1)-1))%MOD*(bigmod((pf-1),MOD-2)))%MOD;
        }
        pf=primes[++pfi];
    }

    if(n!=1) sumofDivs=((sumofDivs*(bigmod(n,m+1)-1))%MOD*(bigmod((n-1),MOD-2)))%MOD;
}
