...
int main()
{
    ...
    for(int i=1;i<=t;i++){
        ...
        double s1=r2+r3,s2=r1+r3,s3=r1+r2;
        double ang1=acos((s2*s2+s3*s3-s1*s1)/(2*s2*s3));
        double ang2=acos((s1*s1+s3*s3-s2*s2)/(2*s1*s3));
        double ang3=acos((s2*s2+s1*s1-s3*s3)/(2*s2*s1));

        double area=0.5*s1*s2*sin(ang3);
        area-=(ang1/(PI*2))*PI*r1*r1;
        area-=(ang2/(PI*2))*PI*r2*r2;
        area-=(ang3/(PI*2))*PI*r3*r3;
