...
int main()
{
    ...
    for(int i=1;i<=t;i++){
        ...
        vii pts(n);
        for(int j=1;j<=n;j++){
            cin>>pts[j-1].ff>>pts[j-1].ss;
        }

        vll temp;
        for(int j=0;j<n;j++){
            for(int k=j+1;k<n;k++){
                double slope,dis=dist2({(double)pts[j].ff,(double)pts[j].ss},{(double)pts[k].ff,(double)pts[k].ss}),c;
                if(pts[j].ff==pts[k].ff){
                    slope=INF;
                    c=-pts[j].ff;
                }
                else{
                    slope=(double)(pts[j].ss-pts[k].ss)*1.0/(pts[j].ff-pts[k].ff);
                    c=pts[j].ss-pts[j].ff*slope;
                }
                vec.pb(mp(mp(slope,dis),c));
            }
        }
        double dis=0,slope=0,c=0;
        ...
        for(int j=0;j<siz(vec);j++){
            if(dis==0 or abs(dis-vec[j].ff.ss)>EPS or abs(slope-vec[j].ff.ff)>EPS){
                c=vec[last].ss;
                cnt=1;
                for(int k=last+1;k<j;k++){
                    if(abs(c-vec[k].ss)>EPS){
                        temp.pb(cnt);
                        cnt=1;
                        c=vec[k].ss;
                    }
                ...
                for(int k=0;k<siz(temp);k++){
                    ans+=tempp*temp[k];
                    tempp+=temp[k];
                }

                temp.clear();
                dis=vec[j].ff.ss,slope=vec[j].ff.ff;
                last=j;
            }
        }
        c=vec[last].ss;
        cnt=1;
        for(int k=last+1;k<siz(vec);k++){
            if(abs(c-vec[k].ss)>EPS){
                temp.pb(cnt);
                cnt=1;
                c=vec[k].ss;
