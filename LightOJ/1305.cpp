...
int main()
{
    ...
    for(int i=1;i<=t;i++){
        ...
        double dx=cx+(ax-bx),dy=ay+(cy-by);

        Vector ad=toVec(Point(ax,ay),Point(dx,dy));
        Vector ab=toVec(Point(ax,ay),Point(bx,by));
        double area=abs(cross(ad,ab));
        cout<<(int)dx<<" "<<(int)dy<<" "<<(int)area<<endl;
