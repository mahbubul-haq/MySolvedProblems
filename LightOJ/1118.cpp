...
int main()
{
   // FI;//FO;
    ...
    for(int i=1;i<=t;i++){
        ...
        Point ctr1,ctr2;
        double r1,r2,ans=0;
        cin>>ctr1.x>>ctr1.y>>r1;
        cin>>ctr2.x>>ctr2.y>>r2;

        double dis=dist(ctr1,ctr2);

        if(dis>=r1+r2 ){
            ans=0;
        }
        else if(dis<EPS or dis<=abs(r1-r2)){
            ans=min(PI*r1*r1,PI*r2*r2);
        }
        else{
            Line l1,l2;
            pointstoLine(ctr1,ctr2,l1);
            l2.a=-2*ctr1.x+2*ctr2.x;
            l2.b=-2*ctr1.y+2*ctr2.y;
            l2.c=ctr1.x*ctr1.x+ctr1.y*ctr1.y-r1*r1-ctr2.x*ctr2.x-ctr2.y*ctr2.y+r2*r2;
            if(abs(l2.b)>EPS){
                l2.a/=l2.b;
                l2.c/=l2.b;
                l2.b=1;
            ...
            Point point;
            areIntersect(l1,l2,point);
            //cout<<point<<endl;

            double dis1=dist(ctr1,point),dis2=dist(ctr2,point);
            if(abs(dis1+dis2-dis)<EPS){

                double ang=acos(dis1/r1);

                ans=(ang/PI)*PI*r1*r1-0.5*r1*r1*sin(ang*2);
                ang=acos(dis2/r2);
                ans+=ang/PI*PI*r2*r2-0.5*r2*r2*sin(ang*2);
            }
            else{
                if(dis1<dis2){
                    swap(dis1,dis2);
                ...
                ans=(ang/PI)*PI*r1*r1-0.5*r1*r1*sin(ang*2);
                ang=acos(dis2/r2);
                ans+=((2*PI-ang*2)/2)*r2*r2+0.5*r2*r2*sin(ang*2);
            }
        }
        cout<<ans<<endl;
