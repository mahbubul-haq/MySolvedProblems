...
/// divs[n]={1,a,b,c,d,n} siz(divs)=6;
/*E(n)=(E(1)+1)/6 + (E(a)+1)/6 + (E(b)+1)/6 +
 *  (E(c)+1)/6 + (E(d)+1)/6+(E(n)+1)/6
 *  all E(n) can be pre-calculated with dp
 *  */
...
int main()
{
    ...
    for(int i=1;i<=t;i++){
        int n; cin>>n;
        cout<<"Case "<<i<<": "<<fixed<<setprecision(6)<<dp[n]<<endl;
    }
