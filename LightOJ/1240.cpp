...
int main()
{
   // FI;FO;
    ...
    for (int tc = 0; tc < t; tc++) {
        ...
        Vector3D ap = toVec(a, p);
        Vector3D ab = toVec(a, b);
        //p.print();
        //a.print();
        //ap.print();

        //cout << magnitude(ap) << " " << magnitude(ab) << endl;

        if (magnitude(ab) < EPS or magnitude(ap) < EPS) {
            cout << dist(a, p) << endl;
            continue;
        }

        double mul = (dot(ap, ab)/ magnitude(ab) / magnitude(ab));
        if (mul < 0)
            mul = 0;
        if (mul > 1)
            mul = 1;
        Point3D aa = translate(a, scale(ab, mul));
        cout << dist(aa, p) << endl;
    }
