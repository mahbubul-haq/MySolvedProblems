...
int main()
{
    ...
    for(;;){
       ...
       // fap(maxn);
        ...
        vector<vector<double> > mat(maxn,vector<double>(maxn+1,0));
        for(int i=0;i<maxn-1;i++) mat[i][maxn]=1;
        mat[maxn-1][maxn]=0;
        for(int i=0;i<maxn-1;i++){
            mat[i][i]=1;

            for(int j=1;j<=6;j++){
                if(i+j<maxn) mat[i][to[i+j]]-=pos;
                else mat[i][to[maxn-1-(i+j-(maxn-1))]]-=pos;
            }
