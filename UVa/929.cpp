...
int main() {
    ...
    while (t--) {
        ...
        PQ<tiii, vector<tiii>, greater<tiii> > q;
        q.push({mat[0][0], 0, 0});
        ...
        while (!q.empty()) {
            auto x = q.top();
            q.pop();
            if (gt(1, x) == n - 1 and gt(2, x) == m - 1)
                break;

            if (dist[gt(1, x)][gt(2, x)] < gt(0, x))
                continue;

            dist[gt(1, x)][gt(2, x)] = gt(0, x);

            for (int i = 0; i < 4; i++) {
                int tox = gt(1, x) + diry[i];
                int toy = gt(2, x) + dirx[i];
                if (tox >= 0 and tox < n and toy >= 0 and toy < m and gt(0, x) + mat[tox][toy] < dist[tox][toy]) {
                    dist[tox][toy] = gt(0, x) + mat[tox][toy];
                    q.push({gt(0, x) + mat[tox][toy], tox, toy});
                }
            }
