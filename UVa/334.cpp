...
int main() {
    ...
    while (cin >> n and n) {
        ...
        reachable.assign(cnt, vb(cnt, false));
        for (int i = 0; i < cnt; i++)
            reachable[i][i] = true;

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < siz(gets[i]); j++) {
                for (int k = j + 1; k < siz(gets[i]); k++) {
                    reachable[saves[gets[i][j]]][saves[gets[i][k]]] = true;
                }
            }
        }
        ...
        while (m--) {
            string from, to;
            cin >> from >> to;
            reachable[saves[from]][saves[to]] = true;
        }
        ...
        if (siz(ans) == 0) {
            ...
            cout << tot << " concurrent events:" << endl;
