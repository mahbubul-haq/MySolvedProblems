...
int main() {
    ...
    while (cin >> n and n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (y == 'Y') {
                if (u == 'U') {
                    mat[0][a - 'A'][b - 'A'] = min(cost, mat[0][a - 'A'][b - 'A']);
                }
                else {
                    mat[0][a - 'A'][b - 'A'] = min(cost, mat[0][a - 'A'][b - 'A']);
                    mat[0][b - 'A'][a - 'A'] = min(cost, mat[0][b - 'A'][a - 'A']);
                }
            }
            else {
                if (u == 'U') {
                    mat[1][a - 'A'][b - 'A'] = min(cost, mat[1][a - 'A'][b - 'A']);
                }
                else {
                    mat[1][a - 'A'][b - 'A'] = min(cost, mat[1][a - 'A'][b - 'A']);
                    mat[1][b - 'A'][a - 'A'] = min(cost, mat[1][b - 'A'][a - 'A']);
                }
            }
        }

        //cout << mat[0][0][0] << " " << mat[1][0][0] << endl;

        for (int ii = 0; ii < 2; ii++) {
            for (int k = 0; k < 26; k++) {
                for (int i = 0; i < 26; i++) {
                    for (int j  = 0; j < 26; j++) {
                        if (mat[ii][i][j] > mat[ii][i][k] + mat[ii][k][j]) {
                            mat[ii][i][j] = mat[ii][i][k] + mat[ii][k][j];
                        }
                    }
                }
            }
        ...
        int mini = maxn;
        for (int i = 0; i < 26; i++) {
            if (mat[0][u - 'A'][i] + mat[1][m - 'A'][i] < mini) {
                mini = mat[0][u - 'A'][i] + mat[1][m - 'A'][i];
            }
        }
