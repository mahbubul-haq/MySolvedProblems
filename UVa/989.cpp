...
void solve(int rr, int cc) {
    ...
    if (mat[rr][cc]) {
        ...
        for (int i = 1; i <= n * n; i++) {
            ...
            if (!check(r[rr], i) and !check(c[cc], i) and !check(blk[rr / n][cc / n], i)) {
                ans[rr][cc] = i;
                r[rr] = r[rr] | (1 << i);
                c[cc] = c[cc] | (1 << i);
                blk[rr / n][cc / n] = blk[rr / n][cc / n] | (1 << i);
                solve(tor, toc);
                r[rr] = r[rr] & ~(1 << i);
                c[cc] = c[cc] & ~(1 << i);
                blk[rr / n][cc / n] = blk[rr / n][cc / n] & ~(1 << i);
...
int main() {
   ...
   //freopen("out.txt", "w", stdout);
    ...
    while (cin >> n) {
        ...
        mat.assign(n * n, vi(n * n));
        blk.assign(n, vi(n, 0));
        r.assign(n * n, 0);
        c.assign(n * n, 0);
        ans.assign(n * n, vi(n * n));
        ok = false;
        ...
        for (int i = 0; i < n * n; i++) {
            for (int j = 0; j < n * n; j++) {
                ...
                if (mat[i][j] > 0) {
                    r[i] = r[i] | (1 << mat[i][j]);
                   // cout << mat[i][j] << " " << i << " " << r[i] << endl;
                    c[j] = c[j] | (1 << mat[i][j]);
                    blk[i / n][j / n] = blk[i / n][j / n] | (1 << mat[i][j]);
                }
            }
            //cout << endl;
        }
