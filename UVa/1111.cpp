...
int main()
{
    ...
    while(cin>>n&&n!=0){
        ...
        vector<Point> ans=convexHull(pts);
        ...
        for(int i=0;i<(int)ans.size()-1;i++){
            ...
            for(int j=0;j<(int)ans.size();j++){
                if(j!=i&&j!=i+1){
                    ...
                    mini1=max(mini1,distToLine(ans[j],ans[i],ans[i+1],temp));
                   // cout<<mini1<<" "<<ans[i]<<" "<<ans[i+1]<<endl;
                    //cout<<ans[j]<<endl;
       ...
       /// mini=100*mini;
        ...
        cout<<"Case "<<cas++<<": "<<fixed<<setprecision(2)<<mini<<endl;
