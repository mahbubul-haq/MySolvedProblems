...
int h, w, cnt;
vector<string> bin, mat;
string ajd = "WAKJSD", ans;
vvi vis, floodVal;
vii points;
bool ok;
set<int> st;
...
void floodFill1(int x, int y) {
    ...
    for (int i = 0; i < 8; i++) {
        int tox = x + dirx[i];
        int toy = y + diry[i];
        if (tox >= 0 and tox < h and toy >= 0 and toy < w and mat[tox][toy] == '0' and !vis[tox][toy]) {
            floodFill1(tox, toy);
        }
    }
}
...
void floodFill(int x, int y) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (tox >= 0 and tox < h and toy >= 0 and toy < w and !vis[tox][toy]) {
            vis[tox][toy] = 1;
            if (mat[tox][toy] == '1') {
                floodFill(tox, toy);
            }
            else if (floodVal[tox][toy] != -1)
                st.insert(floodVal[tox][toy]);
        }
    }
}
...
int main() {
    ...
    while (cin >> h >> w and !(h == 0 and w == 0)) {
        ...
        for (int hh = 0; hh < h; hh++) {
            ...
            for (int i = 0; i < w; i++) {
                if (ro[i] >= 'a') {
                    mat[hh] += bin[ro[i] - 'a' + 10];
                }
                else mat[hh] += bin[ro[i] - '0'];
            }
        }
        ...
        cnt = 0;
        vis.assign(h, vi(w, 0));
        floodVal.assign(h, vi(w, -1));

        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                if (mat[i][j] == '0' and !vis[i][j]) {
                    ok = true;
                    points.clear();
                    floodFill1(i, j);
                    if (ok) {
