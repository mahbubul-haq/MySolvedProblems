...
int main() {
    ...
    while (cin >> r >> c and (r | c)) {
        ...
        queue<tiii> q;
        int ans = 0;
        q.push({0, start.ff, start.ss});

        while (!q.empty()) {
            auto x = q.front();
            q.pop();

            if (gt(1, x) == dest.ff and gt(2, x) == dest.ss) {
                ans = gt(0, x);
                break;
            }

            vis[gt(1, x)][gt(2, x)] = true;
            for (int i = 0; i < 4; i++) {
                int tox = gt(1, x) + diry[i];
                int toy = gt(2, x) + dirx[i];

                if (toy >= 0 and toy < c and tox >= 0 and tox < r and !vis[tox][toy]) {
                    vis[tox][toy] = true;
                    q.push({gt(0, x) + 1, tox, toy});
                }
            }
