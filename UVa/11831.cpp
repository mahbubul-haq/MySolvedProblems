...
int main() {
    ...
    while (cin >> n >> m >> s and (n | s | m)) {
        ...
        for (; i < siz(ins); i++) {
            ...
            if (ins[i] == 'F') {
                int tox = cell.ff + dirx[dir], toy = cell.ss + diry[dir];
                if (tox >= 0 and tox < n and toy < m and toy >= 0 and mat[tox][toy] != '#') {
                    cell = {tox, toy};
                    if (!vis[tox][toy] and mat[tox][toy] == '*')
                        ans++;
                    vis[tox][toy] = 1;
                }
