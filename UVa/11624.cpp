...
int main() {
    ...
    while (t--) {
        ...
        queue<tiii> Joe[2]; //// fire at joe[1]
        ...
        while (true) {
            ...
            while (!temp.empty()) {
                if (now == 0) {
                    ...
                    if (gt(1, x) == r - 1 or gt(2, x) == c - 1 or gt(1, x) == 0 or gt(2, x) == 0) {
                        ans = gt(0, x) + 1;
                        goto loop;
                    }
                    ...
                    for (int i = 0; i < 4; i++) {
                        int tox = gt(1, x) +  diry[i], toy = gt(2, x) + dirx[i];
                        if (tox >= 0 and tox < r and toy >= 0 and toy < c and vis[tox][toy] == 0) {
                            vis[tox][toy] = 1;

                            if (tox == 0 or tox == r - 1 or toy == 0 or toy == c - 1) {
                                ans = gt(0, x) + 2;
                                goto loop;
                            }

                            Joe[now].push({gt(0, x) + 1, tox, toy});
                        }
                    }
                }
                else {
                    ...
                    for (int i = 0; i < 4; i++) {
                        int tox = gt(1, x) +  diry[i], toy = gt(2, x) + dirx[i];
                        if (tox >= 0 and tox < r and toy >= 0 and toy < c and vis[tox][toy] != 2 and vis[tox][toy] != 3) {
                            vis[tox][toy] = 2;
                            Joe[now].push({gt(0, x) + 1, tox, toy});
                        }
                    }
            ...
            if (Joe[now].empty() and Joe[now ^ 1].empty())
                break;
        }
