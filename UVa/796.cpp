...
int n, times, root, numOfchildren;
vii ans;
...
int main() {
    ...
    while (getline(cin, str)) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            st1 >> t;
            while (st1 >> to) {
                if (tak[{min(from, to), max(from, to)}])
                    continue;
                tak[{min(from, to), max(from, to)}] = true;
                g[from].eb(to);
                g[to].eb(from);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            if (dfs_num[i] == INT_MAX) {
                root = i, numOfchildren = 0;
                dfs(i);
            }
        }

        sort(all(ans));
        cout << siz(ans) << " critical links" <<endl;
        for (auto &x : ans)
            cout << x.ff << " - " << x.ss << endl;
        cout << endl;
        getline(cin, str);
