    ...
    for (int tc = 1; tc <= t; tc++) {
        ...
        while (q--) {
            ...
            while (!qu.empty()) {
                ...
                if (x.ss.ff > maxstop)
                    continue;
                if (dist[x.ff][x.ss.ff] < x.ss.ss)
                    continue;
                for (auto &v : g[x.ff]) {
                    if (dist[v.ff][x.ss.ff + 1] > x.ss.ss + v.ss) {
                        dist[v.ff][x.ss.ff + 1] = x.ss.ss + v.ss;
                        qu.push({v.ff, {x.ss.ff + 1, x.ss.ss + v.ss}});
                    }
                }
            }

            int ans = INT_MAX;
            for (int i = 0; i <= maxstop + 1; i++)
