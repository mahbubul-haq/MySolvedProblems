...
/*
 * requirement: kruskal algorithm
 * upto distance r add the new edge values to road and decrease number of states by one for each road
 * for edge values > r add the new added edges to railroads
 */
...
int main() {
    ...
    for (int tc = 1; tc <= t; tc++) {
        ...
        for (int i = 0; i < siz(points); i++) {
            for (int j = i + 1; j < siz(points); j++) {
                edges.eb(dist2D(points[i].ff, points[i].ss, points[j].ff, points[j].ss), i, j);
            }
        }
        sort(all(edges));

        r *= r;
        ld road = 0, rail = 0, states = n;

        for (int i = 0; i < siz(edges); i++) {
            auto x = edges[i];

            if (gt(0, x) <= r) {
                if (Find(gt(2, x)) != Find(gt(1, x))) {
                    parent[Find(gt(1, x))] = Find(gt(2, x));
                    road += sqrtl(gt(0, x));
                    states--;
                }
            }
            else {
                if (Find(gt(1, x)) != Find(gt(2, x))) {
                    parent[Find(gt(1, x))] = Find(gt(2, x));
                    rail += sqrtl(gt(0, x));
                }
            }
        ...
        cout << "Case #" << tc << ": " << states << " " << llround(road) << " " << llround(rail) << endl;
    }
